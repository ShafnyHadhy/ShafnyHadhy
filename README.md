# 👋 Hi, This is **Shafny Hadhy**
### 🚀 Software Engineering Undergraduate | Full-Stack Developer | Tech Enthusiast

I’m an enthusiastic Software Engineering undergraduate at **SLIIT**, passionate about building clean, scalable, and user-focused digital experiences. I enjoy working with **Java**, **MERN**, **PHP**, and modern frontend technologies — crafting everything from websites to full-stack systems.

---

## 🧩 About Me
- 🎓 **BSc (Hons) in Information Technology** — Software Engineering (UG @ SLIIT)  
- 💼 Currently working on: **Personal Finance Tracker (Laravel)**  
- 🌱 Learning: **DevOps**, **Cloud Deployment**, **System Design**  
- 🎯 Goal: Become a strong **Full-Stack Engineer**  
- 💬 Ask me about: MERN, Java, PHP, CSS, UI/UX, system design  
- 🌍 I love **traveling, hiking & adventure**  
- ⚡ Fun fact: I enjoy turning ideas into real projects 😄

---

## 🛠 Tech Stack

### **Languages**
`C` • `C++` • `Java` • `JavaScript` • `PHP` • `SQL`

### **Frontend**
`HTML5` • `CSS3` • `Tailwind CSS` • `React` • `Vite`

### **Backend & Frameworks**
`Node.js` • `Express.js` • `Java EE / Servlets` • `Laravel`

### **Databases**
`MySQL` • `MongoDB`

### **Tools & Other**
`Git` • `GitHub` • `VsCode` • `Postman` • `Figma` • `XAMPP` • `Vercel` • `Netlify`

---

## 🚀 Featured Projects

### 🔹 **CocoSmart – Coconut Plantation Management System (MERN)**
A complete MERN-based management platform with dashboards, AI chatbot, inventory, marketplace, and workforce management.

### 🔹 **Online Banking System (Java)**
Developed using Java Servlets, JSP, DBUtil — includes authentication, bill payments, admin dashboard, and offer management.

### 🔹 **Online Bill & Payment Reminder System**
A web-based solution built using C/C++, PHP, HTML, CSS, JS — handles bill reminders, categories, payments, and staff roles.

---

## 📊 GitHub Stats

![Shafny's GitHub Stats](https://github-readme-stats.vercel.app/api?username=ShafnyHadhy&show_icons=true&theme=tokyonight)  
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=ShafnyHadhy&layout=compact&theme=tokyonight)

---

## 🌐 Connect With Me
📌 **LinkedIn:** *www.linkedin.com/in/shafnyhadhy*
📧 **Email:** *mohamedshafny02@gmail.com*  
🌱 **Portfolio:** *https://shafnyhadhy.vercel.app/*

---

## ✨ Thanks for visiting!
Feel free to explore my work — I’m always building something new 😊
